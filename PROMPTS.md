...
## 8. Eliminating Hardcoded Legacy State
- **Prompt:** "remove all the fallback data and in our website the no data should be hardcoded there ( if not needed ) every thing should be updated accordingly. and just a question shouldnt the model_calls be less to be more effecient ? just a question answer it you do not need to code now"
- **For:** Validating that the dashboard truthfully reflects the LLM's dynamic state and auditing the architectural efficiency of our 100+ microscopic calls.
- **Revised?** We stripped the legacy `recs = []` injection in `mcp/server.py` so the UI clearly states "No recommendations yet" when the LLM is pending, and we mathematically validated that microscopic offline calls are vastly superior to massive batched calls for consumer RAM.

## 9. Dictionary Pathing and Debugging Hallucinations
- **Prompt:** "this is all the data i see when i just start is this not hardcoded... what are the things that llm needs to process like what it is ? is the work llm is doing really important like according to the md is it really needed ?"
- **For:** Justifying the hybrid architecture (Python math + LLM synthesis) to strictly follow the challenge requirements, and catching a massive dictionary pathing bug where `page_keywords` was placed in the wrong dict.
- **Revised?** Yes. The debugging run identified that `server._A.get("page_keywords")` was accidentally querying the root instead of the `clusters` nested dict. Fixing this unlocked the full 40-page entity refinement phase, shooting `model_calls` from 69 to 101. It also proved that our `JSONDecodeError` safety net flawlessly caught local hallucinations without breaking the 11-minute run!