...
def graph_stats(pages, inlinks, graph) -> dict:
    ...
    site_domain = urlparse(pages[0].get("Address", "")).netloc if pages else ""
    ...
    for r in inlinks:
        ...
        if urlparse(dst).netloc != site_domain:
            continue
...
def page_keywords(page, body: str, top=8) -> list[str]:
    """Weighted entity/keyword extraction (deterministic)."""
    title = page.get("Title 1", "") or ""
    h1 = page.get("H1-1", "") or ""
    h2 = (page.get("H2-1", "") or "") + " " + (page.get("H2-2", "") or "")
    body_text = (body or "")[:8000]
    
    weights = Counter()
    for p in _extract_phrases(title): weights[p] += 10
    for p in _extract_phrases(h1): weights[p] += 8
    for p in _extract_phrases(h2): weights[p] += 5
    for p in _extract_phrases(body_text): weights[p] += 1
    
    # Filter out unigrams if there are good bigrams? No, just take top.
    return [w for w, _ in weights.most_common(top)]


def cluster_pages(pages, page_text, n_keywords=12) -> dict:
    """Group indexable pages into topical clusters.

    Updated to use deterministic naming based on hub titles and top keywords.
    """
    idx200 = [p for p in pages if is_html(p) and is_200(p) and indexable(p)]
    ...
    for seg, members in sorted(clusters.items(), key=lambda x: -len(x[1])):
        ...
        cluster_name = None
        if hub:
            page_obj = by_url.get(hub, {})
            title = (page_obj.get("Title 1", "") or "").strip()
            title = re.split(r'\s[|\-]\s', title)[0].strip()
            if 3 < len(title) < 60:
                cluster_name = title.title()
                
        if not cluster_name:
            top_kws = [w for w, _ in ck.most_common(3)]
            cluster_name = " ".join(top_kws).title() if top_kws else seg.replace("-", " ").title()
...
def link_candidates(graph, relate: dict, pages, max_per_page=5, gstats=None, clusters=None) -> list:
    ...
    Updated to deterministically generate high-quality anchors and rank by need.
    """
    idx200 = [p for p in pages if is_html(p) and is_200(p) and indexable(p)]
    inl = {_norm(p["Address"]): _int(p.get("Unique Inlinks")) for p in idx200}
    by_url = {_norm(p["Address"]): p for p in idx200}
    
    orphan_set = set(gstats["orphan_pages"]) if gstats else set()
    underlinked_set = set(gstats["under_linked_pages"]) if gstats else set()
    ...
    for u in important:
        ...
        for e in relate.get(u, []):
            ...
            if v in orphan_set: bonus += 0.5
            if v in underlinked_set: bonus += 0.3
            if v in scattered_set: bonus += 0.2
            
            final_score = round(base_score + bonus, 3)
            scored_targets.append((v, final_score, base_score, e.get("shared", [])))
            
        scored_targets.sort(key=lambda x: -x[1])
        
        for v, final_score, base_score, shared in scored_targets[:max_per_page]:
            target_page = by_url.get(v, {})
            anchor = generate_anchor(target_page, shared)
            reason = f"score={final_score} (rel={base_score}); shared topics: " + ", ".join(shared)
            
            cands.append({"target": v, "relatedness": final_score, "shared_topics": shared,
                          "suggested_anchor": anchor, "reason": reason})
...
def analyze(export_dir: str) -> dict:
    ...
    cands = link_candidates(graph, relate, pages, gstats=gstats, clusters=clusters)
    return {
